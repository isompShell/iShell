...
expect -c "
	...
	expect a\] {
			...
			send \"local-user $CH_USER service-type ssh\n\"
                        send q\n}
	expect i\] {
			send \"ssh user $CH_USER authentication-type password\n\"
			send \"ssh user $CH_USER service-type stelnet\n\"
			send q\n}
	expect \> {send save\n}
	expect continue {
                        send y\n 
